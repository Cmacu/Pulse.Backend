...
namespace Pulse.Matchmaker.Logs {
    public class MatchmakerLogService {
        ...
        public List<MatchmakerLog> ListLogs() {
            return this._context.MatchmakerLogs.Where(x => x.ExpiredAt == null).ToList();
        }
        ...
        private List<MatchmakerLogCounter> GetLogCounters(DateTime fromDate, DateTime toDate) {
            return this._context.MatchmakerLogCounters
        ...
        private List<MatchmakerLogAggregate> GetLogAggregates(DateTime fromDate, DateTime toDate) {
            return this._context.MatchmakerLogAggregates
                .Where(x => x.From >= fromDate && x.To <= toDate)
                .OrderBy(x => x.From).ToList();
        }
        ...
        private void GenerateLogCounters(DateTime from, DateTime? until = null) {
            ...
            var logs = this._context.MatchmakerLogs.Where(x => x.AddedAt >= from && x.AddedAt <= until);
            ...
            this._context.MatchmakerLogCounters.AddRange(logCounters);
            try {
                this._context.SaveChanges();
        ...
        private void GenerateAggregates(DateTime from, DateTime? until = null) {
            ...
            while (from < until) {
                ...
                var logCounters = this._context.MatchmakerLogCounters.Where(x =>
                    x.From == week1 ||
                    x.From == week2 ||
                    x.From == week3 ||
                    x.From == week4
            ...
            this._context.MatchmakerLogAggregates.AddRange(logAggregates);
            try {
                this._context.SaveChanges();
            } catch (DbUpdateException ex) {
                HandleException(ex);
